...
// Écoute rbac-updated pour bascule instantanée des droits
...
// RBAC guard pour actions côté dashboard
watch(isBarcodeMode, (next) => {
  if (next) requirePermission('barcode.create', 'Accès refusé', 'Accès création code-barre non autorisé')
  else requirePermission('qr.create', 'Accès refusé', 'Accès création QR non autorisé')
})
...
// SEO
useHead({
  title: 'Dashboard Admin - QR-Add',
  meta: [
    { 
      name: 'description', 
      content: 'Dashboard administrateur pour la gestion des codes QR et codes-barres' 
    }
  ]
})
</script>
